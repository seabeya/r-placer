...
import { checkInputs, checkImage, buildWorkspaceUrl, getInputsFromUrl } from '@/lib/utils';
...
describe(`fn ${checkInputs.name}`, () => {
  ...
  it('should not throw an error if all inputs are valid', () => {
    expect(() => checkInputs(url, '10', '20')).not.toThrowError;
    expect(() => checkInputs(url, '0', '0')).not.toThrowError;
    expect(() => checkInputs(url, '-10', '-20')).not.toThrowError;
  });

  describe('should throw an error with message -', () => {
    it('-if any input is empty', () => {
      const message = 'Inputs cannot be empty';

      expect(() => checkInputs('', '10', '20')).toThrowError(message);
      expect(() => checkInputs(url, '', '20')).toThrowError(message);
      expect(() => checkInputs(url, '10', '')).toThrowError(message);
    });

    it('-if X or Y is not a number', () => {
      const message = 'X and Y must be numbers';

      expect(() => checkInputs(url, 'abc', '20')).toThrowError(message);
      expect(() => checkInputs(url, '10', 'xyz')).toThrowError(message);
    });

    it('-if X or Y is not an integer', () => {
      const message = 'X and Y must be integers';

      expect(() => checkInputs(url, '10.5', '20')).toThrowError(message);
      expect(() => checkInputs(url, '10', '20.5')).toThrowError(message);
      expect(() => checkInputs(url, '10.5', '20.5')).toThrowError(message);
    });
    ...
    it('-if X or Y is out of range', () => {
      ...
      expect(() => checkInputs(url, upLimit, '20')).toThrowError(message);
      expect(() => checkInputs(url, '10', upLimit)).toThrowError(message);
      expect(() => checkInputs(url, downLimit, '20')).toThrowError(message);
      expect(() => checkInputs(url, '10', downLimit)).toThrowError(message);
    });
  });
});
